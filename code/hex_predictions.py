...
names = []
num_props = len(names) * 3
SA, NSA = load_data_SA_TSNE(dict_hex, names, offset, masking_value)
...
model_predictions_TSNE_seq_hex = model_predict_TSNE_seq(best_batch_size, all_data_TSNE_seq, all_labels_TSNE_seq, best_model_file, best_model)[:-1]

other_output = open(
    "../final_TSNE_seq/hex_predict.txt",
    "w",
    encoding="utf-8",
)
other_output.write(str(model_predictions_TSNE_seq_hex))
other_output.write("\n") 
other_output.close() 
...
model_predictions_TSNE_AP_seq_hex = model_predict_TSNE_AP_seq(num_props, best_batch_size, all_data_TSNE_AP_seq, all_labels_TSNE_AP_seq, best_model_file, best_model)[:-1]

other_output = open(
    "../final_TSNE_AP_seq/hex_predict.txt",
    "w",
    encoding="utf-8",
)
other_output.write(str(model_predictions_TSNE_AP_seq_hex))
other_output.write("\n") 
other_output.close() 
'''
...
model_predictions_TSNE_seq_hex = eval(other_output.readlines()[0])
other_output.close()
...
model_predictions_TSNE_AP_seq_hex = eval(other_output.readlines()[0])
other_output.close()
...
plt.ylabel("AP")
plt.scatter(
    model_predictions_TSNE_seq_hex,
    actual_AP,
    color = '#2e85ff'
)  
slope, intercept, r, p, std_err = stats.linregress(model_predictions_TSNE_seq_hex, actual_AP)
print("t-SNE Seq. model R: " + str(r)) 
print("t-SNE Seq. model corrcoef: " + str(np.corrcoef(model_predictions_TSNE_seq_hex, actual_AP)[0][1])) 
print("t-SNE Seq. model spearmanr: " + str(stats.spearmanr(model_predictions_TSNE_seq_hex, actual_AP)[0])) 
print("t-SNE Seq. model R2: " + str(sklearn.metrics.r2_score(model_predictions_TSNE_seq_hex, actual_AP)))   
mymodel = list(map(myfunc, model_predictions_TSNE_seq_hex)) 
plt.plot(
    model_predictions_TSNE_seq_hex,
    mymodel, color = '#ff120a'
)  
plt.savefig("../final_TSNE_seq/hex_predict.png", bbox_inches="tight")
plt.close() 
...
plt.ylabel("AP")
plt.scatter(
    model_predictions_TSNE_AP_seq_hex,
    actual_AP,
    color = '#2e85ff'
)  
slope, intercept, r, p, std_err = stats.linregress(model_predictions_TSNE_AP_seq_hex, actual_AP)
print("t-SNE Seq. and AP model R: " + str(r)) 
print("t-SNE Seq. and AP model corrcoef: " + str(np.corrcoef(model_predictions_TSNE_AP_seq_hex, actual_AP)[0][1])) 
print("t-SNE Seq. and AP model spearmanr: " + str(stats.spearmanr(model_predictions_TSNE_AP_seq_hex, actual_AP)[0])) 
print("t-SNE Seq. and AP model R2: " + str(sklearn.metrics.r2_score(model_predictions_TSNE_AP_seq_hex, actual_AP)))   
mymodel = list(map(myfunc, model_predictions_TSNE_AP_seq_hex)) 
plt.plot(
    model_predictions_TSNE_AP_seq_hex,
    mymodel, color = '#ff120a'
)  
plt.savefig("../final_TSNE_AP_seq/hex_predict.png", bbox_inches="tight")
plt.close() 
...
plt.savefig("../seeds/all_seeds/TSNE_hex_predict_models_merged.png", bbox_inches="tight")
plt.close() 
...
plt.savefig("../seeds/all_seeds/TSNE_YES_NO_hex_predict_models_merged.png", bbox_inches="tight")
plt.close() 