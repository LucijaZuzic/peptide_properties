...
PRthr = {'../final_no_AI_all/human_AI_predict.txt': 0.37450, '../final_no_AI_seq/human_AI_predict.txt': 0.40009, '../final_no_AI_AP/human_AI_predict.txt': 0.44563,
        "../final_no_AI_TSNE_seq/human_AI_predict.txt": 0.41442, "../final_no_AI_TSNE_AP_seq/human_AI_predict.txt": 0.48019}
ROCthr = {'../final_no_AI_all/human_AI_predict.txt': 0.74304, '../final_no_AI_seq/human_AI_predict.txt': 0.66199, '../final_no_AI_AP/human_AI_predict.txt': 0.68748,
        "../final_no_AI_TSNE_seq/human_AI_predict.txt": 0.65300, "../final_no_AI_TSNE_AP_seq/human_AI_predict.txt": 0.67038}

PRthr = {'../final_no_AI_all/human_AI_predict.txt': 0.320440828, '../final_no_AI_seq/human_AI_predict.txt': 0.32140276000000007, '../final_no_AI_AP/human_AI_predict.txt': 0.22412928599999998,
        "../final_no_AI_TSNE_seq/human_AI_predict.txt": 0.277013928, "../final_no_AI_TSNE_AP_seq/human_AI_predict.txt": 0.30854635999999996}
ROCthr = {'../final_no_AI_all/human_AI_predict.txt': 0.6066449939999999, '../final_no_AI_seq/human_AI_predict.txt': 0.6172134180000001, '../final_no_AI_AP/human_AI_predict.txt': 0.526199318,
        "../final_no_AI_TSNE_seq/human_AI_predict.txt": 0.556881612, "../final_no_AI_TSNE_AP_seq/human_AI_predict.txt": 0.549448272}
...
def read_ROC(test_labels, model_predictions, lines_dict, thrPR, thrROC, name): 
    ...
    plt.savefig(
        '../seeds/all_seeds/' + name + '_ROC_human_AI_no.png',
        bbox_inches="tight",
    )
...
def read_PR(test_labels, model_predictions, lines_dict, thrPR, thrROC, name):  
    ...
    plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.2), ncol=2)
    plt.savefig(
        '../seeds/all_seeds/' + name + '_PR_human_AI_no.png',
        bbox_inches="tight",
    )
...
paths = ["../final_no_AI_AP/human_AI_predict.txt", "../final_no_AI_seq/human_AI_predict.txt", "../final_no_AI_all/human_AI_predict.txt", 
        "../final_no_AI_TSNE_seq/human_AI_predict.txt", "../final_no_AI_TSNE_AP_seq/human_AI_predict.txt"] 
...
for some_path in paths: 
    ...
    model_predictions_human_AI_file_one = open(some_path, 'r')
    model_predictions_human_AI_file_two = open(some_path.replace('final_no_AI_', 'final_no_human_'), 'r')

    model_predictions_human_AI_lines_one = model_predictions_human_AI_file_one.readlines()
    model_predictions_human_AI_lines_two = model_predictions_human_AI_file_two.readlines()

    model_predictions_human_AI_one = eval(model_predictions_human_AI_lines_one[0]) 
    model_predictions_human_AI_two = eval(model_predictions_human_AI_lines_two[0]) 
    model_predictions_human_AI = []
    ind1 = 0
    ind2 = 0
    for i in range(len(df['pep'])):
        if df['expert'][i] == 'AI':
            model_predictions_human_AI.append(model_predictions_human_AI_one[ind1])
            ind1 += 1
        else:
            model_predictions_human_AI.append(model_predictions_human_AI_two[ind2])
            ind2 += 1
    
    print(some_path.split("/")[1])
    print(test_labels, model_predictions_human_AI)
    read_PR(test_labels, model_predictions_human_AI, lines_dict, PRthr[some_path], ROCthr[some_path], names[ind])
    read_ROC(test_labels, model_predictions_human_AI, lines_dict, PRthr[some_path], ROCthr[some_path], names[ind])
...
save_ress = open(DATA_PATH + "final_no_AI_class_human_AI_final.csv", "w")
