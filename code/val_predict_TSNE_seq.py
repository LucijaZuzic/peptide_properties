import numpy as np
from automate_training import load_data_SA_TSNE, model_predict_TSNE_seq, data_and_labels_from_indices, reshape_TSNE_seq, merge_data_TSNE
from utils import getSeed, DATA_PATH, TSNE_SEQ_DATA_PATH
import sys 
...
for seed in seed_list:
	...
	for train_and_validation_data_indices, test_data_indices in kfold_first.split(all_data, all_labels):
		...
		new_path = "../seeds/seed_" + str(seed) + "/TSNE_seq_model_data/TSNE_seq_test_" + str(test_number) + "/"
		...
		for best_params_nr in range(1, 10):
			fold_nr = 0 
			for pair in indices:  
			
				fold_nr += 1  
				base = new_path + "TSNE_seq_test_" + str(test_number) + "_rnn_params_" + str(best_params_nr) + "_fold_" + str(fold_nr)
				...
				#val_data, val_labels = reshape(val_data, val_labels)
		       
				predictions_file = open(predictions_file_name, "r")
				predictions_file_lines = eval(predictions_file.readlines()[0].replace("\n", ""))
				predictions_file.close()
		      
				model_predictions_peptides = model_predict_TSNE_seq(best_batch_size, val_data, val_labels, best_model_file, best_model)
