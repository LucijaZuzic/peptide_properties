import numpy as np
from automate_training import load_data_SA_TSNE, model_training_TSNE_seq, merge_data_TSNE, data_and_labels_from_indices 
from utils import getSeed, DATA_PATH, TSNE_SEQ_DATA_PATH, results_name, log_name, basic_dir
import sys 
...
for train_and_validation_data_indices, test_data_indices in kfold_first.split(all_data, all_labels):
    ...
    if not os.path.exists(basic_dir(TSNE_SEQ_DATA_PATH, test_number)):
        os.makedirs(basic_dir(TSNE_SEQ_DATA_PATH, test_number))

    # Write output to file
    other_output = open(results_name(TSNE_SEQ_DATA_PATH, test_number), "w", encoding="utf-8") 
    other_output.write("")
    other_output.close()

    # Write output to file
    sys.stdout = open(log_name(TSNE_SEQ_DATA_PATH, test_number), "w", encoding="utf-8")

    # Train the ansamble model
    model_training_TSNE_seq(test_number, train_and_validation_data, train_and_validation_labels, kfold_second, EPOCHS, factor_NSA, test_data, test_labels, properties, names, offset, mask_value=masking_value)
