import numpy as np
from automate_training import load_data_SA_AP, merge_data_AP
...
from automate_training import return_callbacks, LEARNING_RATE_SET, reshape_AP
...
train_data, train_labels = reshape_AP(num_props, all_data, all_labels)

#python program to check if a path exists
#if it doesn’t exist we create one
if not os.path.exists("../final_no_human_AP/"):
    os.makedirs("../final_no_human_AP/")

# Write output to file
sys.stdout = open("../final_no_human_AP/training_log.txt", "w", encoding="utf-8")

# Save model to correct file based on number of fold
model_file, model_picture = "../final_no_human_AP/model.h5", "../final_no_human_AP/model_picture.png"

# Choose correct model and instantiate model 

model = new_model.only_amino_di_tri_model(num_props, lstm1=5, lstm2=5, dense=128, dropout=0.5, lambda2=0.0, mask_value=2)
...
other_output = open("../final_no_human_AP/results_accuracy_loss.txt", "w", encoding="utf-8")
other_output.write(
...
other_output = open("../final_no_human_AP/accuracy.txt", "w", encoding="utf-8")
other_output.write(str(accuracy))
other_output.write("\n")
other_output.close()

other_output = open("../final_no_human_AP/loss.txt", "w", encoding="utf-8")
...
plt.savefig("../final_no_human_AP/accuracy.png", bbox_inches="tight")
plt.close()
...
plt.xlabel("Epoch")
plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.1), ncol=2)
plt.savefig("../final_no_human_AP/loss.png", bbox_inches="tight")
plt.close()
