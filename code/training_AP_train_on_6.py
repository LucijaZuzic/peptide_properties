import numpy as np
from automate_training import load_data_SA_AP, model_training_AP, merge_data_AP, data_and_labels_from_indices, extract_len_from_data_and_labels
...
train_and_validation_data_indices = only_one_len_indices
test_data_indices = other_len_indices
...
if not os.path.exists(basic_dir(MY_MODEL_DATA_PATH, test_number)):
    os.makedirs(basic_dir(MY_MODEL_DATA_PATH, test_number))

# Write output to file
other_output = open(results_name(MY_MODEL_DATA_PATH, test_number), "w", encoding="utf-8") 
other_output.write("")
other_output.close()

# Write output to file
sys.stdout = open(log_name(MY_MODEL_DATA_PATH, test_number), "w", encoding="utf-8")

# Train the ansamble model
model_training_AP(num_props, test_number, train_and_validation_data, train_and_validation_labels, kfold_second, EPOCHS, factor_NSA, test_data, test_labels, properties, names, offset, mask_value=masking_value)
