...
import sys  
from automate_training import model_training, merge_data
...
for train_and_validation_data_indices, test_data_indices in kfold_first.split(all_data, all_labels):
    ...
    best_batch_size = model_training(num_props, test_number, train_and_validation_data, train_and_validation_labels, kfold_second, EPOCHS, factor_NSA, mask_value=masking_value)
    best_model_file = MODEL_DATA_PATH+str(test_number)+'_rnn_model_multiple_properties_final_model.h5'
     
    generate_predictions(num_props, best_batch_size, best_model_file, test_number, test_data, test_labels, properties, names, offset, masking_value)
