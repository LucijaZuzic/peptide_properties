...
def read_ROC(test_labels, model_predictions, lines_dict, thrPR, thrROC, name, seed): 
    ...
    plt.savefig(
        '../seeds/all_seeds/' + name + '_ROC_human_AI_' + str(seed) + '.png',
        bbox_inches="tight",
    )
...
def read_PR(test_labels, model_predictions, lines_dict, thrPR, thrROC, name, seed):  
    ...
    plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.2), ncol=2)
    plt.savefig(
        '../seeds/all_seeds/' + name + '_PR_human_AI_' + str(seed) + '.png',
        bbox_inches="tight",
    )
...
for seed in seed_list:
    ...
    for some_path in paths: 
        ...
        df = pd.read_csv(DATA_PATH + "human_AI_results_folds_" + str(seed) + ".csv", sep = ";")
        model_predictions_human_AI_one = df[names[ind]]
        
        read_PR(test_labels, model_predictions_human_AI_one, lines_dict, PRthr[some_path], ROCthr[some_path], names[ind], seed)
        read_ROC(test_labels, model_predictions_human_AI_one, lines_dict, PRthr[some_path], ROCthr[some_path], names[ind], seed)
    ...
    save_ress = open(DATA_PATH + "final_class_human_AI_" + str(seed) + ".csv", "w")
...
save_ress = open(DATA_PATH + "final_class_human_AI_avg.csv", "w")
save_ress.write(ress[-1])
save_ress.close()