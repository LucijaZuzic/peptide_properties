import numpy as np
from automate_training import load_data_SA_seq, merge_data_seq
...
from automate_training import return_callbacks, LEARNING_RATE_SET, reshape_seq
...
train_data, train_labels = reshape_seq(all_data, all_labels)

#python program to check if a path exists
#if it doesn’t exist we create one
if not os.path.exists("../final_no_human_seq/"):
    os.makedirs("../final_no_human_seq/")

# Write output to file
sys.stdout = open("../final_no_human_seq/training_log.txt", "w", encoding="utf-8")

# Save model to correct file based on number of fold
model_file, model_picture = "../final_no_human_seq/model.h5", "../final_no_human_seq/model_picture.png"
...
other_output = open("../final_no_human_seq/results_accuracy_loss.txt", "w", encoding="utf-8")
other_output.write(
...
other_output = open("../final_no_human_seq/accuracy.txt", "w", encoding="utf-8")
other_output.write(str(accuracy))
other_output.write("\n")
other_output.close()

other_output = open("../final_no_human_seq/loss.txt", "w", encoding="utf-8")
...
plt.savefig("../final_no_human_seq/accuracy.png", bbox_inches="tight")
plt.close()
...
plt.xlabel("Epoch")
plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.1), ncol=2)
plt.savefig("../final_no_human_seq/loss.png", bbox_inches="tight")
plt.close()
