import numpy as np
from automate_training import load_data_SA_TSNE, merge_data_TSNE
...
from automate_training import return_callbacks, LEARNING_RATE_SET, reshape_TSNE_seq
...
train_data, train_labels = reshape_TSNE_seq(all_data, all_labels)

#python program to check if a path exists
#if it doesn’t exist we create one
if not os.path.exists("../final_no_human_TSNE_seq/"):
    os.makedirs("../final_no_human_TSNE_seq/")

# Write output to file
sys.stdout = open("../final_no_human_TSNE_seq/training_log.txt", "w", encoding="utf-8")

# Save model to correct file based on number of fold
model_file, model_picture = "../final_no_human_TSNE_seq/model.h5", "../final_no_human_TSNE_seq/model_picture.png"

# Choose correct model and instantiate model 

model = new_model.create_seq_model(input_shape=np.shape(train_data[0]), conv1_filters=5, conv2_filters=5, conv_kernel_size=32, num_cells=4, dropout=0.5, mask_value=2.0)
...
other_output = open("../final_no_human_TSNE_seq/results_accuracy_loss.txt", "w", encoding="utf-8")
other_output.write(
...
other_output = open("../final_no_human_TSNE_seq/accuracy.txt", "w", encoding="utf-8")
other_output.write(str(accuracy))
other_output.write("\n")
other_output.close()

other_output = open("../final_no_human_TSNE_seq/loss.txt", "w", encoding="utf-8")
...
plt.title(
    "Final model seq. props.\n Accuracy"
)
plt.ylabel("Accuracy")
plt.xlabel("Epoch")
plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.1), ncol=2)
plt.savefig("../final_no_human_TSNE_seq/accuracy.png", bbox_inches="tight")
plt.close()
...
plt.xlabel("Epoch")
plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.1), ncol=2)
plt.savefig("../final_no_human_TSNE_seq/loss.png", bbox_inches="tight")
plt.close()
