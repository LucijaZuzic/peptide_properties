...
plt.bar(range(len(new_all_lens.keys())), new_all_lens.values(), label = 'SA')
plt.bar(range(len(new_all_no_lens.keys())), new_all_no_lens.values(), label = 'NSA')
plt.xticks(ticks = range(len(new_all_lens.keys())), labels = new_all_lens.keys())
plt.title("Sample sequences by self-assembly category and length")
plt.xlabel("Length of peptide sequence")
plt.ylabel("Number of sample peptides of a given length")
plt.legend()
plt.savefig(DATA_PATH + "peptide_all.png", bbox_inches="tight")
plt.show()
plt.close()
 
plt.bar(lens.keys(), lens.values(), label = 'SA') 
plt.bar(no_lens.keys(), no_lens.values(), label = 'NSA')
