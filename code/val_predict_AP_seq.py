...
for seed in seed_list:
	...
	for train_and_validation_data_indices, test_data_indices in kfold_first.split(all_data, all_labels):
		...
		new_path = "../seeds/seed_" + str(seed) + "/model_data/all_test_" + str(test_number) + "/"
		...
		for best_params_nr in range(1, 10):
			...
			for pair in indices:  
				...
				fold_nr += 1  
				base = new_path + "all_test_" + str(test_number) + "_rnn_params_" + str(best_params_nr) + "_fold_" + str(fold_nr)
				...
				model_predictions_peptides = model_predict(num_props, best_batch_size, val_data, val_labels, best_model_file, best_model)
